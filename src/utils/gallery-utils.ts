...
export function scanAlbumPhotos(albumId: string): string[] {
	...
	return files.map((f) => {
		const thumbFile = `/gallery/${albumId}/thumb/${f}`;
...
export function getAlbumCover(album: GalleryAlbum, photos: ReturnType<typeof scanAlbumPhotos>): { thumb: string; full: string } {
